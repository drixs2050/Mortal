...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            'Run the Phase 3 low-code DataLoader-worker semantic probe against the confirmed '
    ...
    parser.add_argument(
        ...
        type=int,
        default=[2, 4],
        help='Worker-count candidates to compare against the control.',
    )
    parser.add_argument(
        '--batch-limit',
        type=int,
    ...
    parser.add_argument(
        '--output-json',
        default='artifacts/reports/step6_phase3_worker_semantic_probe.json',
        help='JSON report path.',
    )
    parser.add_argument(
        '--output-md',
        default='artifacts/reports/step6_phase3_worker_semantic_probe.md',
...
def main():
    ...
    for num_workers in args.num_workers:
        overrides = phase3_worker_overrides(num_workers=num_workers)
        ...
        candidate_hashes = collect_batch_hashes(
            ...
            batch_limit=args.batch_limit,
            persistent_workers=bool(dataset_cfg['persistent_workers']),
            prefetch_factor=int(dataset_cfg['prefetch_factor']),
            in_order=bool(dataset_cfg['in_order']),
            multiprocessing_context=str(dataset_cfg['multiprocessing_context']),
        )
        semantic_match, first_mismatch_batch = compare_hashes(control_hashes, candidate_hashes)
        mismatch_index = None
        control_hash = ''
        candidate_hash = ''
        if first_mismatch_batch is not None:
            mismatch_index = first_mismatch_batch - 1
            if mismatch_index < len(control_hashes):
                control_hash = control_hashes[mismatch_index][:16]
            if mismatch_index < len(candidate_hashes):
                candidate_hash = candidate_hashes[mismatch_index][:16]
        rows.append({
            ...
            'status': 'ok' if semantic_match else 'semantic_mismatch',
