#!/usr/bin/env python
"""Phase C: Test gradient accumulation impact — ga=1 vs ga=3, 1000 steps."""
...
def run_variant(*, name, batch_size, grad_accum, base_config, steps,
    ...
    run_dir = ROOT / 'artifacts' / 'tmp' / 'phase_c_grad_accum_test' / name
    ...
    # Steady-state: last 50% of metrics
    ...
    wait_steady = avg_field(recent, 'loader_metrics', 'wait_fraction')
...
def main():
    ...
    # bs=2560 since it fits in VRAM and is faster than bs=2048
    # Compare ga=1 (no accum, eff=5120) vs ga=3 (eff=15360)
    variants = [
        ('ga1_bs2560', 2560, 1),   # eff=5120 — no accumulation
        ('ga3_bs2560', 2560, 3),   # eff=15360 — 3x accumulation
    ]
    ...
    print('\n' + '=' * 110)
    print(f'GRADIENT ACCUMULATION TEST ({args.steps} steps, 512ch/48b 83.4M params)')
    print('=' * 110)
    print(f'{"Config":<18} {"BS":>5} {"GA":>2} {"EffBS":>6} {"SPS":>7} '
          f'{"NLL":>8} {"Top1":>7} {"TopK":>7} {"Wait":>6} {"Wall":>6} {"RC":>3}')
    print('-' * 110)
    for r in results:
    ...
    # Compute speedup
    if len(results) == 2 and all(r['sps_steady'] > 0 for r in results):
        ga1, ga3 = results[0], results[1]
        speedup = ga1['sps_steady'] / ga3['sps_steady']
        print(f'\nSpeedup (ga=1 vs ga=3): {speedup:.2f}x SPS')
        print(f'Effective batch difference: {ga1["eff_global_batch"]} vs {ga3["eff_global_batch"]}')

    output_path = ROOT / 'artifacts' / 'reports' / 'phase_c_probes' / 'grad_accum_test_results.json'
